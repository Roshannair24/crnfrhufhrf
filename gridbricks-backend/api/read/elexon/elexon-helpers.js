const {
  getForecastDataUrl,
  getProdDataUrl,
} = require("./constant/elexon-urls");
const { fuelTypes } = require("./constant/fuelTypes");
...
const getElexonProdData = async ({
  startTime,
  endTime,
  fuelType = fuelTypes.WIND,
}) => {
  try {
    if (!startTime) {
      throw new Error("Invalid startTime");
    ...
    const url = `${getProdDataUrl()}?publishDateTimeFrom=${startTime}&publishDateTimeTo=${endTime}&fuelType=${fuelType}`;
    ...
    console.log("Error at getElexonProdData()", error);
    return [];
  }
};
